...
test('auto-prints with fallback pharmacy metadata when config loading rejects', async () => {
  (getConfigAction as jest.Mock).mockRejectedValueOnce(new Error('config bridge unavailable'));

  render(<ReceiptDetailsModal autoPrint invoice={{
    id: 'sale-fallback', total_amount: 45, created_at: '2026-07-19T12:00:00Z',
    profiles: { full_name: 'Admin' }, patients: null, sales_items: [], payment_method: 'cash',
  }} onClose={() => {}} />);

  await waitFor(() => expect(printHtmlContent).toHaveBeenCalledWith('<html>receipt</html>'));
  expect(generateReceiptHtml).toHaveBeenCalledWith(
    expect.objectContaining({ id: 'sale-fallback' }),
