...
for (const zone of zones) {
  const output = JSON.parse(execFileSync(process.execPath, ['-e', `
    ...
    console.log(JSON.stringify(dates.map(instant => {
      ...
      const row=normalizeDatabaseTimestamps([{created_at:raw,start_time:raw,end_time:raw,date:raw,invoice_date:raw}])[0];
      const local=new Date(instant);
      const localDay=[local.getFullYear(),String(local.getMonth()+1).padStart(2,'0'),String(local.getDate()).padStart(2,'0')].join('-');
      return {instant,raw,row,localDay,parsedDay:new Date(localDay).getDate(),
        dayNumber:local.getDate(),aliasInstant:new Date(row.date).toISOString(),
  ...
  for (const entry of output) {
    ...
    check(`Local business-day default ${zone} ${entry.instant}`, entry.localDay, entry.instant.slice(0,10));
...
const dailySales = finance.match(/SELECT COALESCE\(SUM\(total_amount\), 0\) as total FROM sales_invoices WHERE date\(created_at\) = \? AND status = \?/)[0];
check('Actual finance sales query: Cairo September 1 00:30',100,db.prepare(dailySales).get('2026-09-01','completed').total);
const dailyReturns = finance.match(/SELECT COALESCE\(SUM\(total_refund\), 0\) as total FROM returns WHERE date\(created_at\) = \? AND status = \?/)[0];
check('Actual finance returns query: Cairo September 1 00:45',20,db.prepare(dailyReturns).get('2026-09-01','approved').total);
const returnSource=read('src/app/actions-client/returns.ts');
const predicate=returnSource.match(/\(date\(i.created_at\) = \? OR date\(i.created_at, 'localtime'\) = \?\)/)[0];
// Only substitute the timezone for deterministic simulation, retaining the actual query predicate.
const selectedDates=['2026-08-31','2026-09-01'].filter(date=>db.prepare(`SELECT id FROM sales_invoices i WHERE ${predicate.replace("'localtime'","'+03:00'")}`).get(date,date));
check('Return picker assigns a receipt to exactly its local day',['2026-09-01'],selectedDates);
const revenueSql=read('src/app/actions-client/expenses.ts').match(/SELECT COALESCE\(SUM\(total_amount\), 0\) as revenue\s+FROM sales_invoices\s+WHERE created_at LIKE \? \|\| '%'/)[0];
check('Actual monthly revenue query: September local sale',100,db.prepare(revenueSql).get('2026-09').revenue);
